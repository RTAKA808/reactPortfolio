...
import { Instances, Computers } from './extra/Computers'
  ...
  return (
    <Canvas shadows dpr={[1, 1.5]} camera={{ position: [-1.5, 1, 5.5], fov: 45, near: 1, far: 20 }} eventSource={document.getElementById('root')} eventPrefix="client">
      ...
      <EffectComposer disableNormalPass>
        ...
        <DepthOfField target={[0, 0, 13]} focalLength={0.83} bokehScale={15} height={700} />
      </EffectComposer>
      {/* Camera movements */}
      <CameraRig />
...
function CameraRig() {
  ...
  useFrame((state, delta) => {
    const Position = isMobile ? [0.1, .5, 7] : [0,  (1 + state.pointer.y) / 6, 4.5];
    easing.damp3(state.camera.position, Position, 0.5, delta)
